...
def validate_relationship_fact(fact: RelationshipFact) -> None:
    ...
    if not isinstance(fact.confidence_contribution, (int, float)):
        raise RelationshipSchemaError(
            "RelationshipFact.confidence_contribution must be numeric.\n"
            f"  Received: {fact.confidence_contribution!r}"
        )
