...
def _built_in_rules() -> tuple[CorrelationRule, ...]:
    return (
        ...
        CorrelationRule(
            rule_id="CORR-BRUTE-FORCE-SSH-EXPOSURE",
            title="Brute-force attempt against internal SSH service",
            description="A user authenticated or attempted to authenticate to a host exposing SSH.",
            version="1.0",
            severity=Severity.HIGH,
            confidence=Confidence.MEDIUM,
            mitre=("T1110.001",),
            category="Credential Access",
            tags=("ssh", "brute-force", "internal-exposure"),
            recommendation="Review authentication logs for brute-force patterns and enforce key-based authentication.",
        ...
        CorrelationRule(
            rule_id="CORR-MFA-GAP-IDENTITY",
            title="Privileged Identity Lacks MFA",
            description="An administrative or privileged user does not have MFA configured.",
            version="1.0",
            severity=Severity.HIGH,
            confidence=Confidence.HIGH,
            mitre=("T1078",),
            category="Identity Exposure",
            tags=("admin", "mfa-gap", "identity"),
            recommendation="Enforce MFA registration for all administrative accounts.",
            ...
            conditions=(
                EntityAttributeEqualsCondition("IDENTITY", "admin", True),
                EntityAttributeEqualsCondition("IDENTITY", "no_mfa", True),
            ),
        ),
    )
